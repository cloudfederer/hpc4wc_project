...
def update_halo_mpi(p, copy_stencil, field, num_halo, nx, ny, nz):
    ...
    copy_stencil(
        ...
        dst=field,
        origin={"src": (num_halo, 0, 0), "dst": (num_halo, 0, 0)},
    ...
    copy_stencil(
        ...
        dst=field,
        origin={"src": (num_halo, 0, 0), "dst": (num_halo, ny+num_halo, 0)},
        domain=(nx, num_halo, nz),
    )
...
def update_halo_gt(copy_stencil, field, num_halo):
...
def apply_diffusion(
    p,nx, ny, nz, diffusion_stencil, copy_stencil, in_field, out_field, alpha, num_halo, num_iter=1
):
    ...
    for n in range(num_iter):
        # halo update
        update_halo_mpi(p, copy_stencil, field, num_halo, nx, ny, nz)
...
def main(nx, ny, nz, num_iter, num_halo=2, backend="numpy", plot_result=False):
    ...
    # warmup caches
    apply_diffusion(
        p,nx, ny, nz, diffusion_stencil, copy_stencil, in_field, out_field, alpha, num_halo
    )
